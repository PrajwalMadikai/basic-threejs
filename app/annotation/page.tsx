...
const handleUndo = (): void => {
    ...
    if (action.type === 'measure') {
        ...
        // Remove the last polygon from the state
        setPolygons(prev => prev.slice(0, -1));
    } else if (action.type === 'annotate') {
        // Remove the annotation marker
        if (action.data.objects) {
            action.data.objects.forEach(obj => {
                if (obj.parent) {
                    obj.parent.remove(obj);
        ...
        // Remove the last annotation from the state
    ...
    needsUpdate.current = true;
        ...
        const handleMeasurePoint = (point: THREE.Vector3): void => {
            ...
            setPoints(prevPoints => {
                ...
                if (newPoints.length % 2 === 0 && measureLinesRef.current) {
                    ...
                    // Store all objects in history for proper undo
                    addToHistory({
                        type: 'measure',
                        data: {
                            points: [p1, p2],
                    ...
                    // Reset points array after creating a measurement
                    return [];
                }
        ...
        const handlePolygonPoint = (point: THREE.Vector3): void => {
            ...
            // Track temporary points for the current polygon being created
            ...
            setPoints(prevPoints => {
                ...
                if (newPoints.length >= 2 && polygonGroupRef.current) {
                    // Add line between the last two points
                    const lineMaterial = new THREE.LineBasicMaterial({ color: 0x00ff00 });
                    const p1 = newPoints[newPoints.length - 2];
                    const p2 = newPoints[newPoints.length - 1];
                    const lineGeometry = new THREE.BufferGeometry().setFromPoints([p1, p2]);
                    ...
                    // If we're close to the starting point and have at least 3 points, complete the polygon
                    if (newPoints.length > 2 && p2.distanceTo(newPoints[0]) < 0.2) {
                        // Store all objects that will be part of this polygon for undo purposes
                        const polygonObjects: THREE.Object3D[] = [];
                        
                        // Create points for the completed polygon
                        newPoints.forEach((p) => {
                        ...
                        // Create lines between all points to form the polygon outline
                        ...
                        // Remove the temporary objects used during polygon creation
